...
module.exports = {  // объект настроек
  mode: 'production', // режим работы, их два (development и production(по умолчанию, работает медленнее, в него встроены оптимизаторы кода))
  ...
  module: { // модули для webpack и их настройка
		rules: [ // правила, которые будут действовать для определенных файлов
      {
        ...
        use: { // описываем как и что мы будем использовать
          ...
          options: { // передаем настройки лоадера
            presets: [['@babel/preset-env', { // используем и настраиваем пресет babel
